...
**Concurrency and latency.** `/query` is gated by a `BoundedSemaphore` (`max_concurrent_queries`, default 6) so a burst of simultaneous users degrades to a 429 ("Query capacity is busy") instead of an unbounded pile-up of in-flight BigQuery jobs and model calls; it previously defaulted to 1–2, which serialized nearly all traffic and made the app appear single-user under any concurrent load. Per-request latency was also cut in the model service: `pipeline/text2sql_falkordb.py` used to open a fresh HTTP connection to Ollama and a fresh FalkorDB connection on every call; both now reuse a shared, thread-safe connection (`requests.Session`, a lazily-created graph singleton) across requests. True concurrent SQL *generation* is still bounded by Ollama's own per-model parallelism (`OLLAMA_NUM_PARALLEL`, set to 2 on the laptop serving it, up from the effective default of 1) — raising the app-level semaphore beyond what Ollama can actually run in parallel just queues requests one layer later rather than rejecting them outright, which is still strictly better but is not the same as unlimited parallelism.
